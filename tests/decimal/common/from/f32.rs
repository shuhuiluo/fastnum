macro_rules! test_impl {
    ...
    (COMMON:: 512, $dec: ident, $D: ident, THIS) => {
        ...
        fn test_from_f32_512(#[case] n: f32, #[case] expected: $D) {
            assert!(n.is_normal());
        ...
        #[rstest(::trace)]
        #[case(1.1754942107e-38,    $dec!(1.175494210692441075487029444849287348827052428745893333857174530571588870475618904265502351336181163787841796875e-38))]
    ...
    (COMMON:: 256, $dec: ident, $D: ident, THIS) => {
        ...
        #[rstest(::trace)]
        #[case(f32::MIN_POSITIVE,       $dec!(1.1754943508222875079687365372222456778186655567720875215087517062784172594547e-38), signals![!INEXACT, !ROUND])]
        #[case(1e-13,                   $dec!(9.99999982451670044181213370393379591405391693115234375e-14), signals![])]
        #[case(7.2e-14,                 $dec!(7.20000028022783900016889901962713338434696197509765625e-14), signals![])]
        #[case(1e-16,                   $dec!(1.0000000168623835263871646450439811815158464014530181884765625e-16), signals![])]
        #[case(1e-32,                   $dec!(1.00000002374222799036108273658815415520405083747275818881715403474430559072061e-32), signals![!INEXACT, !ROUND])]
        #[case(1.1754943508e-38,        $dec!(1.1754943508222875079687365372222456778186655567720875215087517062784172594547e-38), signals![!INEXACT, !ROUND])]
        #[case(1.1754947011469036e-38,  $dec!(1.1754947712118268054138578143411206647935049408506700844634832333989024263921e-38), signals![!INEXACT, !ROUND])]
    ...
    (COMMON:: 128, $dec: ident, $D: ident, THIS) => {
        ...
        #[case(1.1754947011469036e-38,  $dec!(1.17549477121182680541385781434112066479e-38))]
        #[case(2.35098744048e-38,       $dec!(2.35098744047595712360210924308786639471e-38))]
        #[case(2.3509889819e-38,        $dec!(2.35098898190426788090088725919040801362e-38))]
        #[case(317e-40,                 $dec!(3.17000000989464355011198160907161547722e-38))]
        fn test_from_f32_128(#[case] n: f32, #[case] expected: $D) {
            assert!(n.is_normal());
            let d = $D::try_from(n).unwrap();
            assert_eq!(d, expected);
        ...
        #[rstest(::trace)]
        #[case(1.1754942107e-38,    $dec!(1.17549421069244107548702944484928734883e-38))]
        #[case(1.0e-39,             $dec!(1.00000021530533325742087560014568310927e-39))]
        #[case(3.92e-39,            $dec!(3.9199993305945648982873957549431278352e-39))]
        #[case(1.0e-40,             $dec!(0.99999461011147595815259190522734994960e-40))]
        #[case(1e-42,               $dec!(1.00052710352791938863954292246900011773e-42))]
        #[case(1.40129846432e-45,   $dec!(1.40129846432481707092372958328991613128e-45))]
        fn test_from_f32_subnormal_128(#[case] n: f32, #[case] expected: $D) {
            assert!(n.is_subnormal());
            let d = $D::try_from(n).unwrap();
            assert_eq!(d, expected);
            assert_eq!(d.op_signals(), signals![!INEXACT, !ROUND]);
    ...
    (COMMON:: 128, $dec: ident, $D: ident) => {
        $crate::decimal::common::from::float::from_float!(FROM f32, $dec, $D);
        $crate::decimal::common::from::float::from_float!(FROM INF f32, $dec, $D);
        $crate::decimal::common::from::float::from_float!(FROM NAN f32, 0b0111_1111_1100_0000_0000_0000_0000_0000, $dec, $D);
        ...
        fn test_from_f32(#[case] n: f32, #[case] expected: $D) {
            let d = $D::try_from(n).unwrap();
            assert_eq!(d, expected);
            assert_eq!(d.op_signals(), signals![]);
        }
    ...
    (UNSIGNED:: 128, $dec: ident, $D: ident) => {
        $crate::decimal::common::from::float::from_float!(FROM U f32, $dec, $D);
    };
    ...
    (SIGNED:: 128, $dec: ident, $D: ident) => {
        $crate::decimal::common::from::float::from_float!(FROM S f32, 32, $dec, $D);
        $crate::decimal::common::from::float::from_float!(FROM NEG INF f32, $dec, $D);

        #[rstest(::trace)]
        #[case(f32::MIN, $dec!(-3.40282346638528859811704183484516925440e+38))]
        fn test_from_f32_signed(#[case] n: f32, #[case] expected: $D) {
            let d = $D::try_from(n).unwrap();
            assert_eq!(d, expected);
            assert_eq!(d.op_signals(), signals![]);
        }
